  ...
  return (
    <div
      className={`transition-all ${
        navbg ? "bg-white opacity-90 shadow-md" : "fixed"
    ...
    >
      <div className="flex items-center justify-between h-full w-[90%] xl:w-[80%] mx-auto">
        ...
        <a href="#" className="flex items-center gap-3">
          ...
          <h1 className="text-lg hidden sm:block md:text-2xl text-blue-800 font-bold">
            TechDev
          </h1>
        </a>
        ...
        <div className="hidden lg:flex items-center space-x-10">
          {navLink.map((link) => (
            <Link
              href={link.url}
              key={link.id}
              className="text-black relative hover:text-rose-500 font-semibold transition-all duration-200 listNav"
            >
              <p> {link.label} </p>
            </Link>
          ))}
